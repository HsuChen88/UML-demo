...
import com.uml.controller.strategy.CanvasEditorContext;
 ...
 */
public class CanvasPanel extends JPanel implements CanvasEditorContext { // 繼承 JPanel，作為可繪製的畫布元件
        ...
        */
        ...
        addMouseListener(new MouseAdapter() { // 註冊滑鼠事件監聽器
            ...
            public void mousePressed(MouseEvent e) { // 滑鼠按下
                ...
                currentStrategy.onPressed(e, CanvasPanel.this); // 委派給目前 strategy 處理
            }

            @Override
            public void mouseExited(MouseEvent e) { // 滑鼠離開畫布時清除 hover 狀態
                clearHover();
            }
            ...
            public void mouseReleased(MouseEvent e) { // 滑鼠放開
                ...
                currentStrategy.onReleased(e, CanvasPanel.this); // 委派給目前 strategy 處理
            }
            ...
            public void mouseClicked(MouseEvent e) { // 滑鼠點擊（pressed + released 在同一位置）
                ...
                currentStrategy.onClicked(e, CanvasPanel.this); // 委派給目前 strategy 處理
            }
        });
        addMouseMotionListener(new MouseMotionAdapter() { // 註冊滑鼠移動事件監聽器
            ...
            public void mouseDragged(MouseEvent e) { // 滑鼠拖曳（按住移動）
                ...
                currentStrategy.onDragged(e, CanvasPanel.this); // 委派給目前 strategy 處理
            }
            @Override
            public void mouseMoved(MouseEvent e) {
                currentStrategy.onMoved(e, CanvasPanel.this); // 委派給 strategy 維護 hover 狀態與模式專屬邏輯
            }
        });
    }
    ...
    @Override
    public void repaintCanvas() { // 實作 CanvasEditorContext：請求畫布重繪
        repaint();
    }

    public void simulateRelease(int x, int y) { // 合成 mouseReleased 並委派給目前 strategy （供 glass pane 使用）
        MouseEvent synthetic = new MouseEvent(this, MouseEvent.MOUSE_RELEASED,
                System.currentTimeMillis(), 0, x, y, 1, false); // 建立合成的滑鼠放開事件
        currentStrategy.onReleased(synthetic, this); // 委派給目前 strategy 處理
    }
