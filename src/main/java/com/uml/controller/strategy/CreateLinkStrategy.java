 ...
 */
public class CreateLinkStrategy implements CanvasMouseStrategy { // 建立連線的 strategy ，對應 Use Case B
    ...
    @Override
    public void onPressed(MouseEvent e, CanvasEditorContext context) { // 滑鼠按下：尋找起點 port（Use Case B 步驟 1）
        // B.1: press must land on a BasicObject port
        sourcePortReference = context.getDocument().findPortReferenceNearPoint(e.getPoint()); // 找出靠近滑鼠的 port reference
        if (sourcePortReference != null) { // 若找到有效起點
            tempEnd = e.getPoint(); // 初始化預覽線的終點
            context.getInteractionState().setTemporaryLink(sourcePortReference, tempEnd); // 設定預覽線狀態
        }
    }

    @Override
    public void onDragged(MouseEvent e, CanvasEditorContext context) { // 滑鼠拖曳：更新預覽線終點
        if (sourcePortReference == null) return; // 若沒有起點則忽略拖曳
        tempEnd = e.getPoint();         // 更新預覽線終點為目前滑鼠位置
        context.getInteractionState().setTemporaryLink(sourcePortReference, tempEnd); // 通知畫布更新預覽線
        context.repaintCanvas(); // 重繪
    }

    @Override
    public void onReleased(MouseEvent e, CanvasEditorContext context) { // 滑鼠放開：嘗試建立連線（Use Case B 步驟 2）
        context.getInteractionState().clearTemporaryLink(); // 清除預覽線
        context.repaintCanvas(); // 重繪
        if (sourcePortReference == null) return; // 若沒有有效起點則放棄

        // B.2: release must land on a *different* BasicObject's port
        PortReference targetPortReference = context.getDocument().findPortReferenceNearPoint(e.getPoint()); // 找出靠近放開位置的 port reference
        ...
        if (targetPortReference != null && targetPortReference.owner() != sourcePortReference.owner()) { // 終點必須存在且不能是起點本身
            ...
            context.execute(new CreateLinkCommand(context.getDocument(), link)); // 執行建立命令並推入歷史（支援 Undo）
        }
